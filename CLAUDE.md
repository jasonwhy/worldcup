...
### 核心哲学
- **机会驱动, 非均衡分配**: 预算作为上限而非强制分配目标
- **正EV首选**: 只投注模型概率超过市场隐含概率的选项 (edge >= min_edge_pct)
- **凯利仓位管理**: 1/4 Kelly 确定每注金额, 大edge大注, 小edge小注
- **RQSPF优先**: 当SPF赔率偏低时, 优先选择让球盘获取黄金赔率区间(1.8-2.5)

### 投资组合规则
| 参数 | 值 | 说明 |
|------|-----|------|
| 预算上限 | 200元 | 非强制花完 |
| Kelly倍数 | 0.25 (1/4) | 保守仓位 |
| 单注最小/最大 | 10元 / 50元 | 双层保护 |
| 每场最多注数 | 2 | 集中度控制 |
| 总注数上限 | 8 | 分散度保证 |
| 方向集中度 | ≤30% | 单方向最大暴露 |
| Edge门槛 | ≥0.5pp | 正期望+安全边际 |
| 赔率区间 | 1.08 - 50.0 | 排除无价值/彩票级 |
| 最低保留 | 5% | 即使无机会也保留 |

### 审计流程
每次生成方案后运行: `python3 engine/audit_betting.py`
三阶段检查: 数据验证(A) → 规则执行(B) → 存档验证(C)
...
- ❌ **凯利仓位不容手动调整**: 模型输出即最终投注金额, 仅可通过 kelly_fraction 全局调整
- ❌ **无Edge投注**: 入选组合的每一注必须有正EV (ev>0) 且 edge>=min_edge_pct
