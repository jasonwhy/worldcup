...
- **实时方向率**：21/33 = **63.6%**（截至 2026-06-21）

### 3. 方案留底铁律
- **`data/bet_plans_archive.json`**：首次生成后存档，每次重生成对比差异
- **永不删除旧方案**：已截止日期显示"⏰ 已截止投注"
- **变更标注**：每次方案更新在顶部显示 📝 横幅（时间戳 + EV 变化）
- **HOT 标签**：推荐度最高的日期自动标注 🔥
...
- ❌ 修改 `bet_plans_archive.json` 中已存档的方案
